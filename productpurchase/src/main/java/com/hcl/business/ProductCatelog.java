...
public class ProductCatelog {
	...
	public List<Product> filterProduct(List<Product> list,String filterCondition, String match){
		...
		case "productType" : filteredProduct =list.stream().filter(x -> x.getProductType().equals(match)).collect(Collectors.toList());
		break;
		case "promotion" : filteredProduct =list.stream().filter(x -> x.getPromotionApplicable() == match.charAt(0)).collect(Collectors.toList());
		break;
		}
